...
import { BLOCK_INFO_URL, BLOCKS_RECENT_URL, BLOCK_SPOTLIGHT_URL } from 'consts';
...
// API Calls
export const getBlockHeight = () => async (dispatch) => ajaxGet(GET_BLOCK_HEIGHT, dispatch, BLOCK_INFO_URL);
...
export const getBlockInfo = (blockHeight) => async (dispatch) =>
  ajaxGet(GET_BLOCK_INFO, dispatch, `${BLOCK_INFO_URL}${blockHeight ? `?height=${blockHeight}` : ''}`);
