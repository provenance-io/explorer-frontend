...
// API Calls
export const getBlockValidators = ({ blockHeight, page = 1, count = 10, sort = 'desc' }) => async (
  dispatch
) =>
  ajaxGet(
...
export const getValidatorsRecent = ({ page = 1, count = 10, status = 'active' }) => async (
  dispatch
) =>
  ajaxGet(
...
export const getAllValidators = ({ page = 1, count = 100, status = 'all' } = {}) => async (
  dispatch
) =>
  ajaxGet(
    GET_ALL_VALIDATORS,
...
export const getValidatorUnbondingDelegations = ({ id, page = 1, count = 10 }) => async (
  dispatch
) =>
  ajaxGet(
