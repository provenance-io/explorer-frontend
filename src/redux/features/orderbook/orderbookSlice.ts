...
** ACTIONS
-------------------*/
export const getDailyPrice = createAsyncThunk(GET_DAILY_PRICE, () =>
  ajax({
    config: {
...
export const getDailyVolume = createAsyncThunk(GET_DAILY_VOLUME, () =>
  ajax({
    config: {
      withCredentials: false,
    },
  ...
  extraReducers(builder) {
    ...
    -------------------*/
      ...
      .addCase(getDailyPrice.fulfilled, (state, { payload }) => {
        ...
        state.dailyPriceFailed = false;
        state.dailyPrice = payload.data.find(
          (ticker: DailyPrice) => ticker.ticker_id === 'HASH_USD'
        );
      })
