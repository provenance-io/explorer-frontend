...
const Forms = ({ 
    ...
    switch (individualConfig.type) {
      ...
      case 'repeating':
        return (
          ...
          <ThisField>
          <Label htmlFor={individualConfig.field} $issub={issub}>{`${capitalize(individualConfig.label)}s:`}</Label>
            <FieldArray
               name={individualConfig.field}
               render={arrayHelpers => (
                 <div>
                   {formik.getFieldProps(individualConfig.field).value && 
                    formik.getFieldProps(individualConfig.field).value.length > 0 && (
                     formik.getFieldProps(individualConfig.field).value.map((value: string, index: number) => (
                       <SubSection key={index}>
                          <SubTitle>
                            {`${capitalize(individualConfig.label)} ${index+1}:`}
                          </SubTitle>
                          {individualConfig.subFields && individualConfig.subFields.map(({ type, label, field: name, dropdown }) => 
                            <FormBuilder
  ...
  return (
    <>
      {config.map((c: MyFieldsProps) => builder(c))}
    </>
  );
};
