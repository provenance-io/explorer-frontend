...
const ContractHistory = () => {
  const { contractHistory, contractHistoryLoading } = useContracts();

  const { matches: isMed } = useMediaQuery(breakpoints.down('md'));

  const { operation, codeId, msg } = contractHistory[0];
  ...
  const getLinkList = (values: string[], prefix: string) => values.map((value) => prefix + value);
  ...
  return (
    <>
      <Content title="Contract History">
        {contractHistoryLoading ? <Loading /> : <Summary data={summaryDataOne} />}
      </Content>
      <Content borderTop="none" borderBottom="none">
        {contractHistoryLoading ? <Loading /> : <Summary data={summaryDataTwo} />}
      </Content>
      <Content borderTop="none">
        {contractHistoryLoading ? <Loading /> : <Summary data={getMsgs} />}
      </Content>
    </>
  );
};
